 ...
 *
 * Dynamic binding (eval_binding, mino_push_bindings, mino_pop_bindings)
 * lives in bindings_dyn.c.  The macro-expander destructuring primitive
 * (prim_destructure) lives in bindings_destr.c.
 */
                                ...
                                const char *ctx)
{
    ...
    for (i = 0; i < plen; i++) {
        ...
        if (sym_eq(p, "&")) {
            ...
            if (i + 1 >= plen) {
                set_eval_diag(S, mino_current_ctx(S)->eval_current_form, "syntax", "MSY003", "& must be followed by a binding form");
                return 0;
            }
            i++;
            p = vec_nth(pattern, i);
            ...
            if (i + 1 < plen && kw_eq(vec_nth(pattern, i + 1), "as")) {
                if (i + 2 >= plen) {
                    set_eval_diag(S, mino_current_ctx(S)->eval_current_form, "syntax", "MSY003", ":as must be followed by a symbol");
                    return 0;
                }
                i += 2;
                p = vec_nth(pattern, i);
            ...
            if (i + 1 < plen) {
                set_eval_diag(S, mino_current_ctx(S)->eval_current_form, "syntax", "MSY003", "unexpected forms after & binding");
                return 0;
            }
            return 1;
        }
                                ...
                                const char *ctx)
{
    ...
    if (val == NULL || mino_type_of(val) != MINO_MAP) {
        val = mino_nil(S);
    }
    ...
    if (as_sym != NULL) {
        if (mino_type_of(as_sym) != MINO_SYMBOL) {
            set_eval_diag(S, mino_current_ctx(S)->eval_current_form, "syntax", "MSY003", ":as must be followed by a symbol");
            return 0;
        }
    ...
    while (mino_is_cons(params)) {
        ...
        if (sym_eq(name, "&")) {
            ...
            if (mino_is_cons(params->as.cons.cdr)) {
                set_eval_diag(S, mino_current_ctx(S)->eval_current_form, "syntax", "MSY003", "& parameter must be last");
                return 0;
            }
...
/* Dynamic binding (eval_binding, mino_push_bindings, mino_pop_bindings)
 * is in bindings_dyn.c.  The macro-expander destructuring primitive
 * (prim_destructure) is in bindings_destr.c. */
