                                  ...
                                  mino_env_t *env)
{
    ...
    if (!mino_is_cons(args)) {
        set_eval_diag(S, mino_current_ctx(S)->eval_current_form, "eval/arity", "MAR001", "host/new requires at least a type argument");
        return NULL;
    }
                                   ...
                                   mino_env_t *env)
{
    ...
    if (!mino_is_cons(args) || !mino_is_cons(args->as.cons.cdr)) {
        set_eval_diag(S, mino_current_ctx(S)->eval_current_form, "eval/arity", "MAR001", "host/call requires target and method arguments");
        return NULL;
    }
                                          ...
                                          mino_env_t *env)
{
    ...
    if (!S->interop_enabled)
        return interop_error(S, "interop disabled");
    if (!mino_is_cons(args) || !mino_is_cons(args->as.cons.cdr)) {
        set_eval_diag(S, mino_current_ctx(S)->eval_current_form, "eval/arity", "MAR001", "host/static-call requires type and method arguments");
        return NULL;
    }
    type_val = args->as.cons.car;
    type_key = kw_name(type_val);
                                  ...
                                  mino_env_t *env)
{
    ...
    if (!S->interop_enabled)
        return interop_error(S, "interop disabled");
    if (!mino_is_cons(args) || !mino_is_cons(args->as.cons.cdr)) {
        set_eval_diag(S, mino_current_ctx(S)->eval_current_form, "eval/arity", "MAR001", "host/get requires target and field arguments");
        return NULL;
    }
    if (mino_is_cons(args->as.cons.cdr->as.cons.cdr)) {
        set_eval_diag(S, mino_current_ctx(S)->eval_current_form, "eval/arity", "MAR001", "host/get takes exactly two arguments");
        return NULL;
    }
    target = args->as.cons.car;
    if (target == NULL || target->type != MINO_HANDLE)
