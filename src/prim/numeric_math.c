...
/* Full-tower double coercion: int, float, float32, bigint, ratio, and
 * bigdec all reduce to a double. as_double only knows long/double,
 * which would reject e.g. (Math/floor 3/2) even though the tower
 * happily divides through it. */
static int tower_as_double(const mino_val *v, double *out)
{
    if (v == NULL) return 0;
