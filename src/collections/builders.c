 ...
 * mino_root so a GC fired mid-build doesn't reclaim the in-flight
...
struct mino_vec_builder { mino_state *S; mino_root *ref; };
struct mino_map_builder { mino_state *S; mino_root *ref; };
struct mino_set_builder { mino_state *S; mino_root *ref; };
...
void mino_vector_builder_push(mino_vec_builder *b, mino_val *v)
{
    mino_val *cur, *next;
    if (b == NULL) return;
    cur  = mino_root_get(b->ref);
    next = mino_conj_bang(b->S, cur, v);
    if (next != NULL && next != cur) {
        mino_root *new_ref = mino_root_new(b->S, next);
        if (new_ref == NULL) return;
        mino_unroot(b->S, b->ref);
        b->ref = new_ref;
    }
}
...
mino_val *mino_vector_builder_finish(mino_vec_builder *b)
{
    mino_val *out;
    if (b == NULL) return NULL;
    out = mino_persistent(b->S, mino_root_get(b->ref));
    mino_unroot(b->S, b->ref);
    free(b);
    return out;
}
