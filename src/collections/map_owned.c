 ...
 *
 * Public API: hamt_assoc_owned, hamt_dissoc_owned, set_conj1_owned,
 * set_disj1_owned (declared in collections/internal.h).
 */
...
/* Owner-tagged set conj. Mirrors set_conj1 (prim/core/collections.c)
 * but routes the HAMT walk and the key_order conj through the owned
 * variants so a transient batch reuses spine nodes and tail-chunk
 * slots in place after the first touch. */
