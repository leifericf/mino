...
(def ^:private lib-srcs
  ["src/eval/eval.c" "src/diag/diag.c" "src/eval/special.c"
   ...
   ;; prim/ is grouped by capability domain, one directory per install
   ;; registry bit (see prim/install.c). prim/core/ is the always-on
   ;; floor plus the registration infrastructure.
   "src/prim/core/prim.c" "src/prim/core/install.c" "src/prim/core/install_stdlib.c"
   "src/prim/core/numeric.c" "src/prim/core/numeric_math.c"
   "src/prim/core/numeric_bit.c" "src/prim/core/numeric_coerce.c"
   "src/prim/core/collections.c" "src/prim/core/collections_transient.c"
   "src/prim/core/bits.c"
   "src/prim/core/sequences.c" "src/prim/core/sequences_seq.c"
   "src/prim/core/lazy.c"
   "src/prim/core/string.c" "src/prim/core/io.c"
   "src/prim/core/http_codec.c" "src/prim/core/http_client.c"
   "src/prim/core/reflection.c" "src/prim/core/meta.c"
   "src/prim/core/stateful.c" "src/prim/core/stateful_bindings.c"
   "src/prim/core/module.c" "src/prim/core/ns.c"
   "src/prim/core/jvm_statics.c"
   "src/prim/codec/url.c" "src/prim/codec/codec.c"
   "src/prim/random/random.c"
   "src/prim/signal/signal.c"
   "src/prim/ws/ws.c" "src/prim/udp/udp.c"
   "src/prim/json/json.c" "src/prim/csv/csv.c"
   "src/prim/toml/toml.c" "src/prim/yaml/yaml.c"
   "src/prim/html/html_entities.c" "src/prim/html/html.c"
   "src/prim/digest/digest.c"
   "src/prim/time/time.c" "src/prim/time/tzdata_blob.c"
   "src/prim/term/term.c"
   "src/prim/path/path.c"
   "src/prim/compress/gzip.c" "src/prim/compress/compress.c"
   "src/prim/archive/zip.c" "src/prim/tar/tar.c"
   "src/prim/regex/regex.c"
   "src/prim/stm/stm.c" "src/prim/agent/agent.c" "src/prim/store/store.c"
   "src/prim/fs/image.c"
   "src/prim/fs/fs.c" "src/prim/proc/proc.c" "src/prim/net/net.c"
   "src/prim/net/tls.c" "src/prim/net/pool.c"
   "src/prim/host/host.c" "src/interop/syntax.c"
...
;; lib-srcs nor this set, because a missing entry means the sanitizer
;; and lean lanes silently skip the file (html.c sat out the lanes
;; this way until a fixup caught it). Keep every entry explained.
(def ^:private lane-excluded-srcs
  #{;; Generated by the gen-stencils pipeline, compiled separately.
    "src/eval/bc/stencils"
    ;; Legacy monolithic TU kept for reference; split into re_compile
...
(let [wired (set lib-srcs)
      on-disk (->> (path/glob "src/**")
                   (filter #(and (str/ends-with? % ".c")
                                 (not (lane-excluded? %))))
                   set)
      unwired (sort (set/difference on-disk wired))]
  (when (seq unwired)
    (throw (ex-info (str "C sources on disk that no lane compiles; "
                         "add them to lib-srcs or lane-excluded-srcs "
                         "with a reason")
                    {:unwired unwired}))))
...
(defn check-stencil-registry
  "G2: cross-check the hardcoded stencil list in gen-stencils against
   ...
   stencils."
  []
  (let [;; Consume the one stencil-list gen-stencils uses, so the G2
        ;; gate cross-checks the single source of truth against disk
        ;; rather than a second copy that could itself drift.
        registry  stencil-list
...
(def ^:private tu-allowlist
  {"src/read/read.c"            "lexer/parser -- inherently sequential, not decomposable"
   "src/prim/core/collections.c" "14 domain primitives in one module, barely over limit"
   "src/prim/agent/agent.c"     "agent subsystem -- worker thread, queue, prims kept together"
   "src/prim/bignum/bigdec.c"   "bigdec tower -- mul/sub/cmp share imath helpers, barely over limit"
   "src/prim/bignum/bignum.c"   "imath wrapper + numeric tower coercions, barely over limit"
   "src/prim/core/module.c"     "ns / require / use / load surface kept together"
   "src/prim/core/ns.c"         "ns primitives -- intern / refer / alias / publics in one module"
   "src/prim/core/numeric.c"    "numeric tower -- arith + compare + math fns sharing coercion helpers"
   "src/prim/core/http_codec.c" "HTTP codec home: parser machine + serializer + redirect + prims, one TU per layer (net.c precedent)"
   "src/prim/core/http_client.c" "HTTP network client: http-request orchestration over the codec, one TU per layer"
   "src/prim/time/time.c"       "time/date layer home: civil core + parsers + formatters + arithmetic + prims, one TU per layer (http codec precedent)"
   "src/prim/core/reflection.c" "reflection / type / meta surface kept together"
   "src/prim/core/sequences.c"  "sequence primitives -- map / filter / take / etc share lazy-seq glue"
   "src/prim/stm/stm.c"         "STM commit + retry + ref/alter/commute/dosync kept together"
   "src/prim/core/string.c"     "string primitives -- per-byte / per-char ops share parsing helpers"
...
(def ^:private fn-allowlist
  #{"src/eval/special.c:eval_impl"     ;; main evaluator dispatch -- inherently large
    ...
    "src/prim/core/module.c:mino_env *env" ;; load_ns_file -- multi-line signature; nested form-by-form loader
    "src/prim/core/module.c:mino_val *prim_require" ;; require -- spec parsing + loading + aliasing in one path
