...
## 2026.08.08-alpha1 — REPL helper refers, apropos ported to Clojure

- Fix: The standalone REPL now refers `clojure.repl` (`doc`, `source`, `dir`, `find-doc`, `pst`, `apropos`) into the `user` namespace at startup, so `(doc map)` works with no manual `require`, matching `clj` (`clojure.main/repl` does the same at its `:init`). Previously `doc` was unbound and `(clojure.repl/doc map)` errored "no var doc in namespace clojure.repl" until a manual `(require 'clojure.repl)`; the macros in `lib/clojure/repl.clj` were registered as bundled source but never loaded into the REPL ns. Root-caused in `main.c` `run_repl` (no `:init`-style refer); embedded envs are unaffected since they never enter `run_repl`. Regression-pinned by a subprocess smoke test that spawns the binary in REPL mode, since the in-process harness never enters `run_repl`.
- Refactor: `apropos` is now a Clojure function in `clojure.repl` walking `(all-ns)`/`ns-publics`, matching `clojure.repl/apropos` in Clojure, instead of a non-referable C primitive. It returns namespace-qualified symbols across all loaded namespaces (previously bare names from only the current namespace plus the caller's lexical scope) and is now a real var, so `(resolve 'apropos)` succeeds, `(dir 'clojure.repl)` lists it, and `:refer` accepts it. The dead C `prim_apropos` env-walk in `src/prim/module.c` is removed; `doc-string` and `source-form` stay C since they read the meta table. No coverage regression (core C primitives are full vars: `cons`, `+`, `map` all appear in `ns-publics`). Parity-pinned by strengthened `apropos` tests and the REPL smoke test.
- Fix: The standalone REPL now also refers `clojure.pprint` (`pp`, `pprint`) into the `user` namespace at startup, so `(pprint x)` works with no manual `require`, matching `clj`'s `clojure.main/repl :init` which requires both `clojure.repl` and `clojure.pprint`. `clojure.pprint` was already 100% implemented (clojure-census 26/26); only the REPL referral was missing. Same `main.c` `run_repl` startup require, standalone-only. Regression-pinned by the REPL smoke test.
...
- Fix: `def` now shadows a referred var instead of erroring, matching Clojure semantics. Previously `eval_def` called `refer_collision_check` which raised a hard `MNS001` error when a `def`/`defmacro`/`declare` name collided with a name brought in via `:refer` from another namespace. Clojure warns and shadows; mino now does the same. The `refer_collision_check` function and its three call sites are removed.
- Add: `mino_text_data` and `mino_text_len` public API functions for reading the byte content of strings, symbols, and keywords without reaching into the internal `mino_val` struct. Embedders that need symbol names (e.g. for completion lists or LSP hover) no longer need internal headers.

## 2026.06.30-alpha1 — mino.store, SLAD image persistence, fs/proc primitives, runtime hardening
...
- Fix: A var in function position now invokes its bound value (Clojure's Var-implements-IFn semantics), so `((resolve 'clojure.string/upper-case) "hi")` calls the fn the var holds instead of throwing "not a function (got var)". The tree-walk call path now unwraps vars the way the bytecode path and `apply` already did.
> This is the first release under calendar versioning (`YYYY.MM.DD[-prerelease]`). It supersedes the prior `0.x` SemVer scheme; see the Versioning section of the README. As an `-alpha` preview, surfaces and formats may change before the stable release on this date.

- Runtime: A var in function position now invokes its bound value (Clojure's Var-implements-IFn semantics), so `((resolve 'clojure.string/upper-case) "hi")` calls the fn the var holds instead of throwing "not a function (got var)". The tree-walk call path now unwraps vars the way the bytecode path and `apply` already did.
- Fix: `binding` now restores `*ns*` across the frame. `*ns*` is backed by the runtime's current-ns field, which `in-ns` mutates directly; the binding frame now snapshots and restores it (on normal exit, on throw unwind, and via the C-API pop), so an `(in-ns ...)` inside a `binding [*ns* *ns*]` body no longer leaks past the frame.
- Fix: `require`/`load` now evaluate the source in a clean top-level env instead of the caller's lexical env. Previously, requiring a namespace from inside a fn whose scope had a local shadowing a clojure.core name (e.g. the task runner's `ns-name`) made every fn defined in the loaded namespace capture that local, so their calls to the global resolved to the local value at runtime and threw "not a function (got string)". A namespace load is a top-level boundary; loaded fns now only see their own namespace's bindings.
- Runtime: `var_registry_add` interns its ns/name arguments internally (idempotent) so a caller passing a non-interned string can no longer silently store a pointer the lookup hash would miss. The two existing callers already interned; this closes the footgun.
- Fix: proc `run` now surfaces a MIN001 error when the output-capture buffer realloc fails, instead of silently dropping the chunk (and every later chunk) and returning a success map with truncated `:out`/`:err`. The child is still reaped and the read ends closed so a blocked writer cannot deadlock the wait.
- Security: proc `run` now closes every inherited descriptor above stderr in the child before `execvp` (bounded by `RLIMIT_NOFILE`), so the exec'd subprocess no longer sees the host's open files, sockets, store handles, or capture pipes. The leak was both an info/capability disclosure into the child and a correctness hazard (a child holding a sibling pipe's write end kept it from reaching EOF).
- Security: `rm-rf` now removes recursively via the `*at` family (`fstatat` with `AT_SYMLINK_NOFOLLOW`, `openat` with `O_NOFOLLOW`, `unlinkat`), pinning each level by its open directory descriptor. This closes the TOCTOU window where a path swapped for a symlink between the stat and the open could redirect the recursion into an unrelated tree (CWE-59). Windows keeps the prior stat-based walk (reparse points differ; POSIX is the test tier).
- Docs: Clarified that `mino.store/listen` fires only for `transact` (and `put`/`retract`); `compact` and `migrate` bypass the tx log and publish directly via `store-commit*`, so they do not notify listeners. The scope is now stated in the `listen`, `compact`, and `migrate` docstrings and pinned by a regression test.
- Fix: SLAD image serializer now emits `wraps_prim` and `template_fn` on the FN line (they were visited as children but never written, so every image orphaned the prim/template as unreferenced lines and the loaded fn lost its prim fast-lane and closure->template back-pointer). The ST line now also emits `watches`, matching the atom/var/tx-ref layout and removing the visit/emit asymmetry (today store->watches is vestigial since stores are not watchable, but the format is now symmetric and forward-ready).
- Security: The `#store[ID VAL]` print form now uses a monotonic per-state counter for the ID instead of the raw heap cell address (`(uintptr_t)v`). The old form leaked the address (information disclosure) and made printed stores nondeterministic across runs; the counter matches how `#agent[ID VAL]` and `#ref[ID VAL]` already identify their cells.
- Fix: SLAD image serializer now emits HAMT maps (any map past the flatmap threshold) with their entries; previously it wrote `M <count>` with no child ids, corrupting every image containing a large map (including every store's db value) and failing the load. Store paths containing spaces or newlines are escaped on the ST line so they survive the whitespace-delimited token format and round-trip on load.
- Fix: SLAD image save now fails loudly (MIO001/MIN001) on OOM or short read while computing the CRC32, instead of writing a `CRC32 00000000` trailer over a body whose real crc differs and reporting success. The value/env ID tables propagate realloc and OOM instead of leaving a NULL field for the next insert to dereference or handing one value two IDs, and load aborts cleanly when an env cannot be GC-rooted for the patch pass.
- Fix: SLAD image loader now warns on stderr (per ADR 17) when it substitutes nil for an unknown type tag, instead of silently dropping the value.
- Fix: store `find-by-range` throws a classified `:range-type-mismatch` error when bounds and attribute values are not mutually comparable, instead of a raw ClassCastException from inside the comparator.
- Fix: proc `run` now fails the child (exit 127, chdir error on captured stderr) when `:dir` cannot be entered, instead of silently running the command in the inherited working directory.
- Security: Replace eval-and-substitute in store/q predicate clauses with resolve-and-apply, closing a code-injection vector where an :in-bound value could run as code in the predicate position or argument slots.
- Fix: Clear store listener registry on close so the process-global atom does not retain closed conns or their listener closures.
- Fix: Resolve migrate :coerce symbols to their var before invocation, so the documented coerce-fn-sym form works (symbols are not callable).
- Fix: Reject store/q :order-by vars not bound by any :where clause instead of silently sorting by nil.
