...
mino's required build toolchain is unchanged and unchangeable: an **ANSI C
compiler** plus **`make`**, with no external dependencies. That is the promise
embedders rely on (`README.md`), and nothing here weakens it. Every task
described below is maintainer-only, optional, and degrades cleanly to "not
available" when its tool is absent.
...
- `zig` is never required: not for bootstrap, not for embedders, not for a
  from-source standalone build.
