...
| `src/prim/time.c` | Time primitives over the own civil core (ADR 21): wall/cpu clocks (now, now-s, cpu-ms; monotonic stays nano-time in io.c), strict epoch-ms <-> time-map converters, parse-time (ISO 8601 / RFC 3339 and the RFC 1123/2822 comma form, untrusted-input surface with position-reporting :time/parse errors), format-time (:iso8601, :iso8601-date, :rfc1123, :rfc2822; no pattern strings), and calendar arithmetic (leap-year?, days-in-month, weekday, add-days, add-months with day clamping, days-between, months-between, human-diff); ungated, years 1..9999, no locale, no zone database |
| `src/prim/path.c` | Path string algebra, pure glob matcher, and the glob walker (ADR 22): join/split/basename/dirname/extension/split-ext/stem/normalize/absolute?/expand-home over plain strings (backslash folds to / on path inputs, patterns never fold), path-glob-match (untrusted-input surface: 256-byte pattern cap plus a backtracking work budget that turns exponential non-match proofs into :eval/bounds), and glob (k_prims_path_fs, installed with the fs capability by mino_install_fs, the file-seq walker precedent): sorted as-given strings, dotfiles hidden unless the segment starts with a dot or :match-dot, wildcard-found symlinks not followed unless :follow-links while literal pattern segments resolve through symlinks, ** whole-segment zero-or-more-dirs, depth capped 1..4096; the pure table is ungated in the floor |
| `src/prim/proc.c` | Process / subprocess primitives |
| `src/prim/net.c` | TCP socket primitives (net-connect, net-read, net-read-all, net-write, net-close) plus the server side (net-listen, net-accept, net-listener-port; listeners are their own handle tag, net-close closes both), the fd bridge the TLS layer pumps records through, and the handle bridge the pool borrows descriptors through |
