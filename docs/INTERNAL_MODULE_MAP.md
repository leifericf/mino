...
| `src/eval_special.c` | ~350 | Eval dispatch (`eval_impl`, `eval`), literal evaluation (symbol/vector/map/set) |
| `src/eval_special_defs.c` | ~170 | Definition special forms: `def`, `defmacro`, `declare` |
...
| `src/eval_special_fn.c` | ~310 | `fn`, `apply_callable`, multi-arity dispatch |
| `src/runtime_state.c` | 516 | State lifecycle (`mino_state_new`, `mino_state_free`), limits, interrupt, env public API (`mino_env_new`, `mino_env_free`, `mino_env_clone`, `mino_new`), refs, fault injection setup |
...
| File | LOC | Responsibility |
|------|-----|----------------|
| `src/prim.c` | ~350 | Shared helpers (`prim_throw_error`, `print_to_string`, seq iterator), `install_core_mino`, `mino_install_core`, `mino_install_io` |
...
| `src/prim_reflection.c` | ~210 | Reflection/utility (`type`, `name`, `eval`, `symbol`, `keyword`, `hash`, `gensym`, `macroexpand`, `throw`, `rand`) |
| `src/prim_meta.c` | ~120 | Metadata (`meta`, `with-meta`, `vary-meta`, `alter-meta!`) |
| `src/prim_regex.c` | ~70 | Regex (`re-find`, `re-matches`) |
| `src/prim_stateful.c` | ~105 | Atoms (`atom`, `deref`, `reset!`, `swap!`, `atom?`) |
| `src/prim_module.c` | ~180 | Module system (`require`, `doc`, `source`, `apropos`, `mino_set_resolver`) |
...
runtime_*.c, mino.c, val.c, vec.c, map.c,
read.c, print.c, clone.c -> mino_internal.h -> mino.h
