...
static mino_val *eval_ok(mino_state *S, mino_env *env, const char *src)
{
...
static void test_vector_conj_bang(mino_state *S, mino_env *env)
{
    /* Start from an empty persistent vector, grow via conj! in a loop,
     * seal with persistent, and verify the count plus a spot check. */
    mino_val *v0 = mino_vector(S, NULL, 0);
    mino_val *t  = mino_transient(S, v0);
    long long   i;
    mino_val *final;
    long long   n;
    (void)env;
    ...
    for (i = 0; i < 256; i++) {
        mino_val *elem = mino_int(S, i);
        mino_val *t2   = mino_conj_bang(S, t, elem);
    ...
    {
        mino_val *t2 = mino_conj_bang(S, t, mino_int(S, -1));
...
static void test_vector_assoc_bang(mino_state *S, mino_env *env)
{
    mino_val *v0 = eval_ok(S, env, "[10 20 30 40]");
    mino_val *t  = mino_transient(S, v0);
    mino_val *t2;
    mino_val *final;
...
static void test_vector_pop_bang(mino_state *S, mino_env *env)
{
    mino_val *v0 = eval_ok(S, env, "[1 2 3 4 5]");
    mino_val *t  = mino_transient(S, v0);
    mino_val *t2;
    mino_val *final;
    (void)env;
...
static void test_map_assoc_bang(mino_state *S, mino_env *env)
{
    mino_val *m0 = eval_ok(S, env, "{}");
    mino_val *t  = mino_transient(S, m0);
    long long   i;
    mino_val *final;
    REQUIRE(t != NULL, "map: transient creation failed");
    for (i = 0; i < 50; i++) {
        mino_val *k = mino_int(S, i);
        mino_val *v = mino_int(S, i * 10);
        mino_val *t2 = mino_assoc_bang(S, t, k, v);
...
static void test_map_dissoc_bang(mino_state *S, mino_env *env)
{
    mino_val *m0 = eval_ok(S, env, "{:a 1 :b 2 :c 3}");
    mino_val *t  = mino_transient(S, m0);
    mino_val *k  = mino_keyword(S, "a");
    mino_val *t2;
    mino_val *final;
...
static void test_set_conj_disj_bang(mino_state *S, mino_env *env)
{
    mino_val *s0 = eval_ok(S, env, "#{}");
    mino_val *t  = mino_transient(S, s0);
    mino_val *t2;
    mino_val *final;
    long long   i;
...
static void test_type_mismatch(mino_state *S, mino_env *env)
{
    /* mino_transient on a non-collection must fail. */
    mino_val *t = mino_transient(S, mino_int(S, 42));
    (void)env;
    REQUIRE(t == NULL, "type mismatch: transient on int should fail");
}

static void test_invalid_after_persistent(mino_state *S, mino_env *env)
{
    mino_val *v = mino_vector(S, NULL, 0);
    mino_val *t = mino_transient(S, v);
    mino_val *p;
    mino_val *t2;
    (void)env;
